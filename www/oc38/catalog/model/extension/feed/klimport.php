<?php
class ModelExtensionFeedUniXml extends Model {
  ...
  public function getProducts($filter_data) {
    ...
    if($query->num_rows){
      ...
      foreach($query->rows as $key=> $product){
        ...
        if(!$attribute_status && $feed != 'google'){ //если не скрыто
          ...
          foreach($product_attributes->rows as $p_attribute){
            $product['attributes'][] = array(
              ...
              'text'  => $this->ClearReplace(htmlspecialchars($p_attribute['text'], ENT_QUOTES, 'UTF-8'), $replacename)
            );
          }
        }
        //attributes
        ...
        if($multiply_options_status && $option_multiplier_id) {
          ...
          $option_sql = "SELECT ovd.name as value, od.name, pov.price_prefix, pov.price, pov.quantity, pov.option_value_id
    ...
    foreach($products as $product){
      ...
      // $product['description'] = $this->ClearReplace(htmlspecialchars($product['description'], ENT_HTML5, 'UTF-8'), $replacename, $clear_desc);
      ...
      $product['price'] = round($product['price'] * $currency); //$currency
      if($product['special']){
        $product['special'] = round($product['special'] * $currency);
      }
