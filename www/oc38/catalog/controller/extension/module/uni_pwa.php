	...
	private function setManifest() {
		...
		if (!file_exists($manifest)) {
			...
			foreach($img_sizes as $key => $size) {
				...
				$icons .= ' {"src":"'.$this->getImg($size).'", "type":"image/png", "sizes":"'.$size.'x'.$size.'"'. ($key + 1 == count($img_sizes) ? '}' : '},');
			}
		
			$data = '{';
			$data .= '"dir":"ltr",';
			$data .= '"lang":"'.$this->language->get('code').'",';
			$data .= '"name":"'.$uniset['pwa']['name'][$lang_id].'",';
			$data .= '"short_name":"'.$uniset['pwa']['short_name'][$lang_id].'",';
			$data .= '"scope":"/",';
