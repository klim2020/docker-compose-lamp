...
class ControllerExtensionModuleUniNewData extends Controller {
	...
	private function getHeaderData() {
		...
		if($contacts_main) {
			...
			foreach($contacts_main as $key => $contact) {
				...
				if($number) {
					$href = $this->createContactHref($contact['type'][$lang_id], $number);
					...
					if($key == 1 || isset($contact['is_second'][$lang_id])) {
						...
						if(!$contact['icon'][$lang_id] && substr($number, 0, 1) == '+') {
							//$contact['icon'][$lang_id] = 'fas fa-phone';
						}
		...
		if(!isset($uniset['wishlist']['disabled'])) {
			$wishlist_products = isset($this->session->data['wishlist']) ? $this->session->data['wishlist'] : [];
	...
	private function getFooterData() {
		$this->load->language('extension/module/uni_othertext');
		$this->load->language('extension/module/uni_pwa');
		...
		if(substr($this_route, 0, 7) == 'account' || substr($this_route, 0, 9) == 'affiliate') {
			...
			if(isset($uniset['dadata']['account_page']['status']) && $uniset['dadata']['token']) {
		...
		if(isset($uniset['show_special_timer'])) {
			$this->document->addScript($dir_script.'special-timer.js');
		}
		...
		if(in_array($this_route, $uni_routes)) {
			if($uniset['catalog']['cat_description']['height']) {
		...
		if($uniset['fly_contacts']['status'] && isset($uniset['fly_contacts']['items'])) {
			
			$data['fly_contacts'] = [
				'tooltip' => $uniset['fly_contacts']['tooltip'][$lang_id],
				'class'   => $uniset['fly_contacts']['status'] == 2 ? 'hidden-xs' : ($uniset['fly_contacts']['status'] == 3 ? 'visible-xs' : '')
			];
			
			foreach($uniset['fly_contacts']['items'] as $fly_contact) {

				if(isset($fly_contact['number'][$lang_id])) {
				
					$href = $this->createContactHref($fly_contact['type'][$lang_id], $fly_contact['number'][$lang_id]);

					$data['fly_contacts']['items'][] = [
						'text' 		=> $fly_contact['text'][$lang_id],
						'href'		=> $href,
						'number'	=> $fly_contact['number'][$lang_id],
						'icon' 		=> $fly_contact['icon'][$lang_id],
						'img' 		=> $fly_contact['img'][$lang_id]
					];
				}
			}
		}
						...
						foreach ($headerlink2['children'] as $child) {
							if(isset($child['title'][$lang_id])) {
									...
									foreach ($child['children'] as $child2) {
										if(isset($child2['title'][$lang_id])) {
											$children2_data[] = [
												...
												'sort_order'=> $child2['sort_order'][$lang_id]
											];
										}
									}
									
									if(count($children2_data) > 1) {
										array_multisort(array_column($children2_data, 'sort_order'), SORT_ASC, $children2_data);
									}
								}
								...
								$children_data[] = [
									...
									'sort_order'=> $child['sort_order'][$lang_id],
									'children'	=> $children2_data
								];
								
								if(count($children_data) > 1) {
	...
	private function getCatalogData() {
		...
		$data['subcategory_mobile_view'] = $uniset['catalog']['subcategory']['mobile_view'];
	...
	public function wishlistRemove() {
		...
		if ($product_info && in_array($product_id, $this->session->data['wishlist'])) {
			$this->load->model('account/wishlist');
	...
	private function createContactHref($type, $number) {
		$href = '';
