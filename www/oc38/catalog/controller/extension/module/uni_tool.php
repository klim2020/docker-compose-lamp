...
class ControllerExtensionModuleUniTool extends Controller {
	...
	private function setGeneratedStyle($generated_file) {
		...
		//fly contacts
		if($uniset['fly_contacts']['status']) {
			$style .= '--fly-contacts-btn-c:#'.$uniset['fly_contacts']['color']['btn_color'].';--fly-contacts-btn-bg:#'.$uniset['fly_contacts']['color']['btn_bg'].';';
			$style .= '--fly-contacts-bg:#'.$uniset['fly_contacts']['color']['block_bg'].';';
			$style .= '--fly-contacts-item-c:#'.$uniset['fly_contacts']['color']['icon_color'].';--fly-contacts-item-bg:#'.$uniset['fly_contacts']['color']['icon_bg'].';';
		}
		...
		*/
		...
		$style .= '@media (max-width:575px){';
			...
			$subcategory_mobile_view = $uniset['catalog']['subcategory']['mobile_view'];
			
			if($subcategory_mobile_view == 'select') {
				$style .= '.category-list{display:none}';
			} else if ($subcategory_mobile_view == 'imghide') {
				$style .= '.category-list__item{align-items:center;background:rgba(0 0 0 / .03);border:none}.category-list__img{display:none}';
			}
