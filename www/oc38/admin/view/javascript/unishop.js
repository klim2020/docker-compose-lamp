...
$(function() {
	...
	function addHeaderLinks2Sub(lang_id, parent_key, elem) {
		...
		html  += '<input type="text" name="uni_set[header][headerlinks2]['+parent_key+'][children]['+key+'][sort_order]['+lang_id+']" value="" placeholder="'+uni_placeholder_sort+'" class="form-control" />';
	...
	function addHeaderLinks2Sub2(lang_id, parent_key, parent_key2, elem) {
		...
		html  += '<input type="text" name="uni_set[header][headerlinks2]['+parent_key+'][children]['+parent_key2+'][children]['+key+'][sort_order]['+lang_id+']" value="" placeholder="'+uni_placeholder_sort+'" class="form-control" />';
		html  += '<span class="input-group-btn btn-default" onclick="$(this).parent().parent().remove();" title="'+uni_text_delete+'"><i class="fa fa-close"></i></span>';
		html  += '</div>';
		html  += '</div>';
	...
	function addMainPhones(lang_id, elem) {
		...
		html  += '<div class="menu-icon__title"></div>';
		html  += '<div>';
		html  += '<input type="text" name="uni_set[header][contacts][main]['+key+'][text]['+lang_id+']" value="" placeholder="'+uni_text_text+'" class="form-control" />';
		html  += '<input type="text" name="uni_set[header][contacts][main]['+key+'][number]['+lang_id+']" value="" placeholder="'+uni_text_tel_link+'" class="form-control" />';
	...
	function addContacts(lang_id, elem) {
		...
		html  += '<div class="menu-icon__title"></div>';
		html  += '<div>';
		html  += '<input type="text" name="uni_set[header][contacts][addit]['+key+'][text]['+lang_id+']" value="" placeholder="'+uni_text_text+'" class="form-control" />';
		html  += '<input type="text" name="uni_set[header][contacts][addit]['+key+'][number]['+lang_id+']" value="" placeholder="'+uni_text_link+'" class="form-control" />';
...
///////////////////
	...
	function addFlyContacts(lang_id, elem) {
		let key = 1, data_key = $('#fly-contacts-'+lang_id+' .fly-contacts-wrap').last().data('key');
		
		if(data_key) {
			key = data_key+1;
		}

		html  = '<div class="fly-contacts-wrap" data-key="'+key+'">';
		html  += '<div class="fly-contacts">';
		html  += '<div>';
		html  += '<div class="menu-icon__title"></div>';
		html  += '<ul class="nav nav-tabs">';
		for (i in uni_img_or_ico_arr) {
			html  += '<li class="m '+(i == 'img' ? 'active' : '')+'">';
			html  += '<a href="#fly-contacts-type-'+key+'-'+lang_id+'-'+i+'" data-toggle="tab">'+uni_img_or_ico_arr[i]+'</a>';
			html  += '</li>';
		}
		html  += '</ul>';
		html  += '<div class="tab-content">';
		for (i in uni_img_or_ico_arr) {
			html  += '<div id="fly-contacts-type-'+key+'-'+lang_id+'-'+i+'" class="tab-pane menu-icon '+(i == 'img' ? 'active' : '')+'">';
			if (i == 'img') {
				html  += '<a href="" id="thumb-image_fly-contacts-items-'+key+'-'+lang_id+'" data-toggle="image" class="img-thumbnail img">';
				html  += '<img src="'+uni_img_placeholder+'" alt="" title="" data-placeholder="'+uni_img_placeholder+'" style="max-width:100px;max-height:100px" />';
				html  += '</a>';
				html  += '<input type="hidden" name="uni_set[fly_contacts][items]['+key+'][img]['+lang_id+']" value="" id="image_fly-contacts-items-'+key+'-'+lang_id+'" />';
			} else {
				html  += '<a id="f-m-item-icon-'+key+'-'+lang_id+'" class="menu-icon__i" onclick="popup_icons($(this).attr(\'id\'))">';
				html  += '<i class="fa fa-plus-circle"></i>';
				html  += '</a>';
				html  += '<input type="hidden" name="uni_set[fly_contacts][items]['+key+'][icon]['+lang_id+']" value="" />';
			}
			html  += '</div>';
		}
		html  += '</div>';
		...
		html  += '<div class="menu-icon__title"></div>';
		html  += '<div>';
		html  += '<input type="text" name="uni_set[fly_contacts][items]['+key+'][text]['+lang_id+']" value="" placeholder="'+uni_text_tooltip+'" class="form-control" />';
		html  += '<input type="text" name="uni_set[fly_contacts][items]['+key+'][number]['+lang_id+']" value="" placeholder="'+uni_text_tel_link+'" class="form-control" />';
		html  += '<select name="uni_set[fly_contacts][items]['+key+'][type]['+lang_id+']" class="form-control">';
		html  += '<optgroup label="По клику:" />';
		html  += '<option value="">Ничего</option>';
		for (i in uni_contact_actions) {
			html  += '<option value="'+i+'">'+uni_contact_actions[i]+'</option>';
